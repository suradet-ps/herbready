...
fn config_dir() -> Result<PathBuf> {
  let dir = dirs::data_local_dir()
    ...
    .join("HerbReady");
  fs::create_dir_all(&dir).with_context(|| format!("ไม่สามารถสร้าง directory {}", dir.display()))?;
  Ok(dir)
}
...
fn get_master_key() -> Result<String> {
  let machine_id =
    whoami::fallible::devicename().unwrap_or_else(|_| "HerbReady-DefaultMachine".to_string());
  let username = whoami::username();
  Ok(format!(
    "HerbReady-v2-MasterKey-{}-{}",
    username, machine_id
  ))
}
...
pub fn read_db_config() -> Result<DatabaseConfig> {
  ...
  let content = match fs::read_to_string(&path) {
    ...
    Err(e) => {
      return Err(e).with_context(|| format!("ไม่สามารถอ่านไฟล์ config.ini ที่ {}", path.display()));
    }
  };
